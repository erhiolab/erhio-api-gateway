...
// ReleaseNodeRequest 请求结束后释放节点活跃数
...
// selectNodeByLoad 根据加权活跃请求数选择节点
func selectNodeByLoad(nodes []*models.WeightedNode) *models.ServiceNode {
	...
	candidates := make([]*models.WeightedNode, 0, len(nodes))
	for _, node := range nodes {
		if len(candidates) == 0 {
...
// compareNodeLoad 比较两个节点的加权活跃请求数
...
// runtimeNodesMatch 检查当前运行时节点映射是否与服务节点匹配
...
// rebuildRuntimeNodes 重建构建运行时节点映射
